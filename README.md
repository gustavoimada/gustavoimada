# Hey, I'm Gustavo Imada 👋

I'm a Computer Science student focused on **full-stack development**, **backend systems**, **databases** and **low-level programming**.

I like building projects that connect theory with practical software: web applications, REST APIs, database-driven systems, IoT prototypes, data structures, algorithms and C programs that explore how operating systems work under the hood.

## 🚀 Main Focus

- Full-stack web development with **React**, **Vue**, **Node.js** and **Spring Boot**
- Backend development with **REST APIs**, authentication, persistence and business rules
- Databases with **PostgreSQL**, **MongoDB** and SQL modeling
- Low-level programming with **C**, **Linux**, process scheduling, filesystems and data structures

## 🛠️ Tech Stack

- **Languages:** C · C++ · Java · JavaScript · SQL
- **Frontend:** React · Vue
- **Backend:** Node.js · Express · Spring Boot · JPA · JWT
- **Databases:** PostgreSQL · MongoDB · Oracle
- **Tools & Concepts:** Git · Linux · Maven · REST APIs · Data Structures · Algorithms · Operating Systems · IoT

## ⭐ Featured Projects

| Project | Stack | What it shows |
|---|---|---|
| [PureAir](https://github.com/gustavoimada/pureair) | ESP32, Arduino/C++, Node.js, PostgreSQL | IoT prototype for monitoring air-conditioner filter health with sensor readings, backend persistence and alert logic. |
| [Ativo Operante](https://github.com/gustavoimada/ativo-operante) | Vue, Spring Boot, JPA, JWT, PostgreSQL | Full-stack web application for reporting and tracking urban complaints, with authentication and relational persistence. |
| [React Clinic Scheduler](https://github.com/gustavoimada/react-clinic-scheduler) | React, Node.js, Express, MongoDB | Clinic appointment scheduler with patient management, appointment validation and frontend/backend integration. |
| [Linux File System Simulator](https://github.com/gustavoimada/linux-file-system-simulator) | C, Linux, Debian, POSIX | Terminal application that simulates a Linux filesystem using inodes, blocks, permissions, hard links and symlinks. |
| [Sorting Algorithms with Files and Linked Lists](https://github.com/gustavoimada/sorting-algorithms-files-and-linked-lists) | Java, binary files, linked lists | Classic sorting algorithms applied to file-based structures and linked lists, including interactive visualizations. |
| [DentalFX](https://github.com/gustavoimada/dentalfx) | JavaFX, Maven, PostgreSQL, JasperReports | Desktop system for dental clinic scheduling, patient management, appointments, reports and database integration. |

## 🧠 Systems, Algorithms & Low-Level Projects

| Project | Focus |
|---|---|
| [Process Lifecycle Simulator](https://github.com/gustavoimada/process-lifecycle-simulator) | C terminal simulator using linked lists, Round Robin scheduling, FIFO queues, priority handling and execution reports. |
| [JavaScript Program Interpreter](https://github.com/gustavoimada/js-program-interpreter) | Lightweight JavaScript interpreter written in C, with lexical analysis, parsing and source-code execution. |
| [Huffman Tree](https://github.com/gustavoimada/huffman-tree) | C++ implementation of Huffman encoding and decoding using binary trees and ordered linked lists. |

## 🌐 Additional Application Project

| Project | Focus |
|---|---|
| [PlayMySongs](https://github.com/gustavoimada/play-my-songs) | Spring Boot application for music registration, search and playback with MongoDB. |

## 📚 Currently Improving

- Advanced backend architecture and API design
- Database modeling and query optimization
- Linux, operating systems and filesystem concepts
- Data structures, algorithms and performance analysis
- Full-stack applications with cleaner documentation and deployment-ready structure

## 📫 Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat&logo=linkedin)](https://linkedin.com/in/guimada)
[![Email](https://img.shields.io/badge/Email-red?style=flat&logo=gmail)](mailto:gu.imada@hotmail.com)
